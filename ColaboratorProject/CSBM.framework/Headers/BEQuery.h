...
/**
 The `BEQuery` class defines a query that is used to query for `PFObject`s.
 */
@interface BEQuery<PFGenericObject : BEObject *> : NSObject<NSCopying>

///--------------------------------------
#pragma mark - Blocks
///--------------------------------------

typedef void (^BEQueryArrayResultBlock)(NSArray<PFGenericObject> *_Nullable objects, NSError * _Nullable error);
...
/**
 ...
 @return The same instance of `BEQuery` as the receiver. This allows method chaining.
 */
...
- (instancetype)whereKey:(NSString *)key
            nearGeoPoint:(BEGeoPoint *)geopoint
...
/**
 Returns a `BEQuery` that is the `or` of the passed in queries.

 @param queries The list of queries to or together.

 @return An instance of `BEQuery` that is the `or` of the passed in queries.
 */
...
/**
 Add a constraint that requires that a key's value matches a `BEQuery` constraint.
...
/**
 Add a constraint that requires that a key's value to not match a `BEQuery` constraint.
